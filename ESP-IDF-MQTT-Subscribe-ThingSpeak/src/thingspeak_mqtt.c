...
{
  ESP_LOGD("MQTT", "Event dispatched from event loop base=%s, event_id=%d", base, (int) event_id);
...
void thingspeak_mqtt_client_publish(thingspeak_mqtt_client_t* client_struct)
{
  ...
  esp_mqtt_client_publish(client_struct->handle,topic,field_data,0,0,0);
